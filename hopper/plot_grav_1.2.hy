...
; find y-displacement at 0.75

(defn eval-parabola [coeffs x]
  (+ (* (get coeffs 0) x x) (* (get coeffs 1) x) (get coeffs 2)))

(for [i (range 0 (len all-coeffs))]
  (setv coeffs (get all-coeffs i))
  (setv y-at-075 (eval-parabola coeffs 0.75))
  (print (conju ["For test n=" (+ i 1) ", the displacement at t=0.75s is " (round y-at-075 2) " cm."])))
...
(plot-adv
    ...
    "Displacement (cm)"
    "Projectile Motion"
    "A projectile (n=1) is launched vertically upward. The displacement over time is shown with a fitted parabolic curve."
    (conju [outdir "/f1.2_plot.png"])
    xlim
    ylim)